...
import _3_probabilty.FileWalker;
import _3_probabilty.Prob;
 ...
 */
public class ProbabilityCalculatorForPostives {

    private static final String P_UNIGRAM_FILE_NAME_OUTPUT = "files/step4/1gramOUTPUTWithProbs.txt";
    private static final String P_BIGRAM_FILE_NAME_OUTPUT = "files/step4/2gramOUTPUTWithProbs.txt";

    public static final String UNIGRAM_FILE = "files/step4/1gramOUTPUT.txt";
    public static final String BIGRAM_FILE = "files/step4/2gramOUTPUT.txt";

    public static final String FILES_STEP3_NEGATIVE_SITUATION_PHRASES_WITH_PROB_TXT = "files/step5/PositiveSentimentPhrasesWithProb.txt";
    public static final String FILES_STEP3_NEGATIVE_SITUATION_PHRASES_TXT = "files/step5/PositiveSentimentPhrases.txt";
    ...
    private void calculateProbabilities(HashMap<String, TokenDetails> unigramsMap, HashMap<String, TokenDetails> bigramsMap, HashMap<String, TokenDetails> trigramsMap) {
        ...
        while (it2.hasNext()) {
            ...
            String key = pair.getKey();
            Prob prob = fileWalker.calculateProbabilty(key);
            pair.getValue().setProb(Float.parseFloat(prob.getProbabilty().toString()));
            pair.getValue().setCount(prob.getTotalOccurence().intValue());
            if (i % 10 == 0) {
                System.out.print(".");
            }
        }
    ...
    public HashMap<String, TokenDetails> compareProbs(HashMap<String, TokenDetails> unigramsMap, HashMap<String, TokenDetails> bigramsMap, HashMap<String, TokenDetails> trigramsMap) {
        ...
        while (it.hasNext()) {
            Map.Entry<String, TokenDetails> pair = (Map.Entry<String, TokenDetails>) it.next();
            if (pair.getValue().getCount() > 15) {
                if (pair.getValue().getProb() >= 0.6) {
                    if (Utility.isVerb(pair.getValue().getString()) &&  !pair.getValue().getString().contains("#")) {
    ...
    public static void main(String... strings) {
        ProbabilityCalculatorForPostives p = new ProbabilityCalculatorForPostives();
        //p.getProbabilities();
        p.getHighestProb();
    }
}
